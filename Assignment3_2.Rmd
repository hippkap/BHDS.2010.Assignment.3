...
# First, we will want to install the appropriate packages "tidyr", "ggplot2",
# "tidyverse" and "dplyr", and then utilize the library() function to load in 
# the packages and their dependencies.

# We also will name an object "text" and read the .csv file
...
# Visualization 1 (and Bonus Plot) 
# Branch: Boxplot_Visualization
# Branch: Bonus_Plot
...
# Visualization 2
# Branch: BarChart_and_SummaryStats
# Branch: T_test
...
# Summary Statistics and Paired T-test
...
# *AS A BONUS*: We can conduct a paired t-test to see if the decreasing in 
# texting behavior among the two groups was statistically significant. 
# We chose a paired t-test as each participant has repeated measures. 
...
# have a statistically significant decline in texts during the observational 
# period.We are 95% confident the true mean difference lies between 4.64 and 
# 19.12.
...
# the alpha = 0.05 significance level. Thus, we can conclude Group 2 did not 
# have a statistically significant decline in texts during the observational 
# period.We are 95% confident the true mean difference lies between -0.1036 and 
# 7.6236. Note that 0 is included in this interval, meaning there is some   
# probability that no decline in texting behavior occurs. 

# Overall, these results suggest that Group 1 showed a significant reduction in 
# texting behavior over time, whereas Group 2’s change was smaller and 
# nonsignificant.
...
# Furthermore, these conclusions are supported by the paired t-test results, 
# which revealed that Group 1’s decrease in texting frequency was statistically 
# significant (p = 0.0024), whereas Group 2’s smaller reduction was not 
# (p = 0.0559). This suggests that the observed decline in Group 1 reflects a 
# genuine behavioral change over time, while Group 2’s messaging patterns 
# remained comparatively stable.

# Reflection: # Through this collaborative GitHub project, we learned to 
# effectively use branching, merging, and version control in a real data 
# workflow. Communication and version history management were critical to 
# keeping our analyses organized and reproducible.
